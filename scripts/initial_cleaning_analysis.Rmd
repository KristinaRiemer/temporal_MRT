...
dim(singleyear_allcoords)
range(singleyear_allcoords, na.rm = TRUE)
singleyear_allcoords[1, 77]
```
```{r}
singleyear_subsetcoords = temp_variable_all[715:720, 350:360, 1380]
singleyear_subsetcoords
```

```{r}
singleyear_subsetcoords2 = temp_variable_all[0.25, 89.75, 1380]
singleyear_subsetcoords2
```
```{r}
singleyear_subsetcoords3 = temp_variable_all[0:50, 0:50, 1380]
singleyear_subsetcoords3
range(singleyear_subsetcoords3, na.rm = TRUE)
```

#### TODO

* Plot all temps for a single year to determine hemisphere locations
...
```{r}
#temp_variable_all[0:720, 0:360, 1380]
#temp_variable_all[210:220, 90:100, 1380]
df_test = expand.grid(1:359, 1:719)
#df_test = expand.grid(0:360, 0:720)
names(df_test) = c("lat", "lon")
df_test$temp = NA
df_test
```
...
for(i in 1:nrow(df_test)){
  df_test[i, "temp"] = temp_variable_all[df_test$lon[i], df_test$lat[i], 1380]
}
df_test
```
```{r}
min(df_test$temp, na.rm = TRUE)
max(df_test$temp, na.rm = TRUE)
df_test %>% 
  filter(lon == 320 & lat == 120)
```

```{r}
ggplot(df_test, aes(lon, lat)) +
  geom_tile(aes(fill = temp)) +
  geom_vline(xintercept = 230) +
  geom_hline(yintercept = 244)
```
Actual dataset orientation. 
```{r}
ggplot(df_test, aes(lon, -lat)) +
  geom_tile(aes(fill = temp)) +
geom_vline(xintercept = 230) +
  geom_hline(yintercept = -244)
```
Flipped correct dataset orientation. 
...
* Look up all July (?) temperatures for each site
* Test Portal temps against the on-site temperature measurements

#### Notes

* Conversions: 
    * grid_lon = coord_lon * 2
    * grid_lat = -((coord_lat - 90) * 2)
...
ggplot(df_test, aes(lon, lat)) +
  geom_tile(aes(fill = temp)) +
  geom_vline(xintercept = lon_conversion(-109)) +
  geom_hline(yintercept = lat_conversion(31))
```
