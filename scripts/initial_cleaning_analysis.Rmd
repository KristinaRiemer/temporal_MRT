...
```{r}
temp_variable_all[site_locations$grid_lon[1], site_locations$grid_lat[1], 1375]
```
Extract July 2014 temperature for Portal. 
```{r}
...
month_indices = data.frame(index = seq(1, 1380), month = rep(months, 115), year = years)
month_indices
```

Generate list of July temperature index values. 
```{r}
july_indices = month_indices %>% 
  filter(month == "Jul") %>% 
  select(index)
july_indices
```
...
```{r}
extract_single_temp = function(year_index){
  single_temp = temp_variable_all[site_locations$grid_lon[1], site_locations$grid_lat[1], year_index]
  return(single_temp)
}
july_temps2 = mapply(extract_single_temp, year_index = july_indices$index)
july_temps2
```
Extract all July temperatures for Portal and put into dataframe. 
