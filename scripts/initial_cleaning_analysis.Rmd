...
* For each species at each site, plot mean yearly mass against mean annual temp (?)
* Get p-value and r-squared for each species
* Calculate r from r-squared
* Plot histogram of r values for each site
* Portal first
* Generalize to all sites
...
for(site in sites){
  #get temp dataset
  #get masses dataset
    #input for Portal = by_sp_yr; equivalent for fray jorge? 
    #rename
  #combine temp and species masses
  #put all site dataframes together with new site column
  #plot all species temp vs mass for each site
}
```
Generalize getting and plotting temp vs mass for all sites. 
```{r}
for(site in sites){
  #get tidy and glance of lm for all species
  #combine these dataframes
  #calculate r value
  #plot r distributions, one per site
}
```
Generalizing getting and plotting r distributions for all sites. 
```{r}
portal_annual_temps = annual_temps %>% 
  filter(site == "portal") %>% 
  data.frame() %>% 
  select(-avg_temp_sd, -site)
portal_annual_temps
```
Get temp dataset for Portal. 
```{r}
portal_annual_masses = by_sp_yr %>%
  add_tally() %>% 
  filter(n > 5) %>% 
  select(-mass_sd)
portal_annual_masses
```
Get mass dataset for Portal, including removing species with fewer than five years of masses. 
```{r}
portal_annual_masses = portal_annual_masses %>% 
  left_join(portal_annual_temps, by = c("yr" = "year"))
portal_annual_masses
```
Get temp for each species in each year. 
```{r}
ggplot(portal_annual_masses, aes(x = avg_temp, y = mass_mean)) +
  geom_point() +
  stat_smooth(method = "lm") +
  facet_wrap(~species, scales = "free")
```
Plot linear regression between annual temp and mean mass for each species. Each
point corresponds to a year of data. 
```{r}
portal_lm_tidy = portal_annual_masses %>% 
  nest(-species) %>% 
...
portal_lm_tidy
```
Produce dataframe of linear regression results for Portal species with p-value 
and slope. 
```{r}
portal_lm_glance = portal_annual_masses %>% 
  nest(-species) %>% 
  mutate(fit = map(data, ~ lm(mass_mean ~ avg_temp, data = .)), 
...
portal_lm_glance
```
Produce dataframe of linear regression results for Portal species with r2. 
```{r}
portal_lm = portal_lm_glance %>% 
  full_join(portal_lm_tidy, by = "species")
portal_lm
```
...
portal_lm
```
Combine r-squared and slope to get r value. 
```{r}
ggplot(portal_lm, aes(x = r)) +
  geom_histogram() +
  xlim(c(-1, 1))
ggplot(portal_lm, aes(x = r)) +
  geom_density() +
