...
* Look up all monthly temperatures for each site
* Get average annual temperature for each site
...
monthly_temps = data.frame(index = seq(1, 1380), month = rep(months, 115), year = years)
monthly_temps$month = factor(monthly_temps$month, levels = month.abb)
monthly_temps
```
Set up dataframe with month and year for each index value. 
```{r}
#extract_single_temp = function(lon, lat, month_index){
#  single_temp = temp_variable_all[lon, lat, month_index]
#  return(single_temp)
#}
```
Write function to get Portal temps based on desired month. 
```{r}
#monthly_portal_temps = mapply(extract_single_temp, lon = site_locations$lon[1], lat = site_locations$lat[1], month_index = monthly_temps$index)
#monthly_temps$portal = monthly_portal_temps
#monthly_temps
```
Extract all monthly temperatures for Portal and put into dataframe. 

```{r}
test_df = c(site = character(), month = numeric(), each_temp = numeric())
for(site in site_locations$site_name){
  ...
  for(month in monthly_temps$index){
    each_temp = temp_variable_all[lon, lat, month]
    #print(c(site, lon, lat, month, each_temp))
    row = c(site, month, each_temp)
    test_df = rbind(test_df, row)
  }
}
head(test_df)
tail(test_df)
...
annual_temps = monthly_temps %>%
  group_by(year) %>% 
  summarize(portal = mean(portal))
annual_temps
```
Get mean annual temperatures for Portal and put into dataframe. 

Set up to do for all sites in locations list. 
