...
```{r}
extract_single_temp = function(month_index){
  single_temp = temp_variable_all[site_locations$grid_lon[1], site_locations$grid_lat[1], month_index]
  return(single_temp)
}
```
Write function to get Portal temps based on desired month. 
```{r}
monthly_portal_temps = mapply(extract_single_temp, month_index = monthly_temps$index)
