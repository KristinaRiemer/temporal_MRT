...
* Choose what columns and column names each dataset should have once it's cleaned
...
```{r}
ggplot(data = by_sp_yr_portal, aes(x = yr, y = mass_mean)) +
...
```{r}
ggplot(data = by_sp_yr_fray_jorge, aes(x = year, y = mass_mean)) +
  geom_point() +
  geom_errorbar(aes(ymin = mass_mean - mass_sd, ymax = mass_mean + mass_sd)) +
...
```{r}
for(each_site in unique(all_annual_masses$site)){
  print(ggplot(all_annual_masses[all_annual_masses$site == each_site,], aes(x = avg_temp, y = mass_mean)) +
          geom_point() +
          stat_smooth(method = "lm") +
          facet_wrap(~species, scales = "free"))
}
```
Plot each species average mass and temperature per year for all sites. 
```{r}
for(site in sites){
  #input = all_annual_masses
