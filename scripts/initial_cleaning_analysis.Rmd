...
* Add test for UTM to lat/lon conversion for Portal? 
...
#### TODO 
* Convert coordinates from UTM to lat/lon
* Exchange UTM coordinates for lat/lon in locations dataframe
...
#               "../data/raw_portal/ecy1360-sup-0001-datas1")
...
```{r}
portal_mean_east = mean(portal_coords$east) #lon
portal_mean_north = mean(portal_coords$north) #lat
#portal_mean_UTM = SpatialPoints(c(portal_mean_east, portal_mean_north), proj4string = ("+proj=utm +zone= +datum=WGS84"))
```
Generate variables for each coordinate direction. 
Convert UTM to lat/lon. Expect Portal to be about -109.063, 31.9107. 
UTM zone 12
```{r}
site_locations = site_locations %>% 
  bind_rows(c(site_name = "Portal", 
              lon = portal_mean_east, 
              lat = portal_mean_north))
site_locations
```
